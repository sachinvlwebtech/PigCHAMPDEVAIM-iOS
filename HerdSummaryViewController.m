...
//        [tracker send:[[GAIDictionaryBuilder createScreenView] build]];
...

        {
            ...
            NSArray* resultArray1 = [[CoreDataHandler sharedHandler] getTranslatedText:[[NSMutableArray alloc] initWithObjects:@"OK",@"Please Wait...",@"You must be online for the app to function.",@"Your session has been expired. Please login again.",@"Server Error.",@"Signing off.",@"Herd Summary",nil]];
                ...
                for (int i=0; i<7; i++) {
                    if (i==0){
                        ...
                        if ([dictMenu objectForKey:[@"Herd Summary" uppercaseString]] && ![[dictMenu objectForKey:[@"Herd Summary" uppercaseString]] isKindOfClass:[NSNull class]]) {
                            if ([[dictMenu objectForKey:[@"Herd Summary" uppercaseString]] length]>0) {
                                self.title = [dictMenu objectForKey:[@"Herd Summary" uppercaseString]]?[dictMenu objectForKey:[@"Herd Summary" uppercaseString]]:@"";
                            }
                        }
                    }
                }
    ...
    @try {
        ...
        if (indexPath.row==0 || indexPath.row==12) {
         ...
         //   lblTitle.text = @"Herd size";
           lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Herd Size"]];
            }else {
            lblDetails.text = [dict valueForKey:@"Boars"];
           // lblTitle.text = @"Boars";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Boars"]];
            }
        }else  if (indexPath.row==1 || indexPath.row==5){
            if (indexPath.row==1) {
                lblDetails.text = [dict valueForKey:@"Gilts"];
                // lblTitle.text = @"Gilts";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Gilts"]];
            }else {
                lblDetails.text = [dict valueForKey:@"Sows"];
               //  lblTitle.text = @"Sows";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Sows"]];
            }
        }else {
            if (indexPath.row==2) {
                lblDetails.text = [dict valueForKey:@"Retained"];
               // lblTitle.text = @"Retained";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Retained"]];
            }else if (indexPath.row==3){
                lblDetails.text = [dict valueForKey:@"Available"];
               // lblTitle.text = @"Available";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Available"]];
            }else if (indexPath.row==4){
                lblDetails.text = [dict valueForKey:@"Served"];
              //  lblTitle.text = @"Served";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Served"]];
            }else if (indexPath.row==6){
                lblDetails.text = [dict valueForKey:@"Dry (Weaned)"];
               // lblTitle.text = @"Dry (Weaned)";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Dry (Weaned)"]];
            }else if (indexPath.row==7){
                lblDetails.text = [dict valueForKey:@"Assumed In-Pig"];
              //  lblTitle.text = @"Assumed In-Pig";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Assumed In-Pig"]];
            }else if (indexPath.row==8){
                lblDetails.text = [dict valueForKey:@"Not In-Pig"];
             //   lblTitle.text = @"Not In-Pig";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Not In-Pig"]];
            }else if (indexPath.row==9){
                lblDetails.text = [dict valueForKey:@"Aborted"];
             //   lblTitle.text = @"Aborted";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Aborted"]];
            }else if (indexPath.row==10){
                lblDetails.text = [dict valueForKey:@"Lactating"];
              //  lblTitle.text = @"Lactating";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Lactating"]];
            }else if (indexPath.row==11){
                lblDetails.text = [dict valueForKey:@"Nurse Sows"];
              //  lblTitle.text = @"Nurse Sows";
                lblTitle.text =[dict valueForKey:[NSString stringWithFormat:@"%@translatedValue",@"Nurse Sows"]];
            }
        }
        ...
        [ServerManager sendRequest:[NSString stringWithFormat:@"token=%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"token"]] idOfServiceUrl:23 headers:nil methodType:@"GET" onSucess:^(NSString *responseData) {
            ...
            if ([responseData isEqualToString:@"\"User is not signed in or Session expired\""] || [responseData localizedCaseInsensitiveContainsString:@"\"Token not found\""])
            {
               ...
               NSDictionary *dictResponse = [NSJSONSerialization JSONObjectWithData:[responseData dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
                ...
                arrHerd = [dictResponse valueForKey:@"_HerdSummary"];
                ...
                for (NSDictionary *dictRp in arrHerd ) {
                    if (![[dictRp valueForKey:@"prop"] isKindOfClass:[NSNull class]]) {
                        ...
                        [dict setValue:[dictRp valueForKey:@"translatedValue"] forKey:[NSString stringWithFormat:@"%@translatedValue",[dictRp valueForKey:@"prop"]]];
                    }
                }
