        ...
        if (resultArray1!=0) {
            ...
            for (int i=0; i<11; i++){
                if (i==0) {
                    ...
                    if ([dictMenu objectForKey:[@"Cache has updated." uppercaseString]] && ![[dictMenu objectForKey:[@"Cache has updated." uppercaseString]] isKindOfClass:[NSNull class]]) {
                        if ([[dictMenu objectForKey:[@"Cache has updated." uppercaseString]] length]>0) {
                            strCatchUpdated = [dictMenu objectForKey:[@"Cache has updated." uppercaseString]]?[dictMenu objectForKey:[@"Cache has updated." uppercaseString]]:@"";
                        }
                    }
                }
            }
        ...
        [ServerManager sendRequest:srvUrl idOfServiceUrl:10 headers:nil methodType:@"GET" onSucess:^(NSString *responseData) {//dict
            ...
            if ([responseData isEqualToString:@"\"User is not signed in or Session expired\""] || [responseData localizedCaseInsensitiveContainsString:@"\"Token not found\""])
            {
                ...
                if (preRec.intValue !=[strRec integerValue]) {//Need to unit test
                    [self displayToastWithMessage:strCatchUpdated];
//                    UIAlertController *myAlertController = [UIAlertController alertControllerWithTitle:@"PigCHAMP"
