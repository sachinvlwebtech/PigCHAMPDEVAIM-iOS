...
//                    }
                    ...
                    if ([dictMenu objectForKey:[@"HISTORY" uppercaseString]] && ![[dictMenu objectForKey:[@"HISTORY" uppercaseString]] isKindOfClass:[NSNull class]]) {
                        if ([[dictMenu objectForKey:[@"HISTORY" uppercaseString]] length]>0) {
                            strHistoryTitle = [dictMenu objectForKey:[@"HISTORY" uppercaseString]]?[dictMenu objectForKey:[@"HISTORY" uppercaseString]]:@"";
                        }else{
                    ...
                    [self.btnHistry setTitle:strHistoryTitle forState:UIControlStateNormal];
                }else  if (i==6){
...
//                    }
                    ...
                    if ([dictMenu objectForKey:[@"Are you sure you want to delete this?" uppercaseString]] && ![[dictMenu objectForKey:[@"Are you sure you want to delete this?" uppercaseString]] isKindOfClass:[NSNull class]]) {
                        if ([[dictMenu objectForKey:[@"Are you sure you want to delete this?" uppercaseString]] length]>0) {
                            strSureDelete = [dictMenu objectForKey:[@"Are you sure you want to delete this?" uppercaseString]]?[dictMenu objectForKey:[@"Are you sure you want to delete this?" uppercaseString]]:@"";
                        }
                    }
                }
            }
...
//        }
        if (![[[arrSummary objectAtIndex:indexPath.row] valueForKey:@"translatedValue"] isKindOfClass:[NSNull class]]){
            lblName.text = [[arrSummary objectAtIndex:indexPath.row] valueForKey:@"translatedValue"]?[[arrSummary objectAtIndex:indexPath.row] valueForKey:@"translatedValue"]:@"";
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)cv cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    @try {
        ...
        if (indexPath.section==0){
            ...
            if (isSow) {
                if (indexPath.row==0){
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Date"];
                }
                else if (indexPath.row==1) {
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Parity"];
                }
                else if (indexPath.row==2){
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Event"];
                }
                else if (indexPath.row==3){
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Detail"];
                }
                else if (indexPath.row==4){
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Location"];
                }
                else{
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Operator"];
                }
            }else{
                if (indexPath.row==0){
                    cellobj.lblHeader.text = [self getTranslatedTextForString:@"Date"];
...
//                                                             [self.clHistory reloadData];
                                                             ...
                                                             {
                                                                 UIAlertController *myAlertController = [UIAlertController alertControllerWithTitle:@"PigCHAMP"
                                                                                                                                            message:[self getTranslatedTextForString:responseData]
                                                                                                                                     preferredStyle:UIAlertControllerStyleAlert];
                                                                 UIImageView *logoImageView = [[UIImageView alloc] initWithFrame:CGRectMake(35, 7, 40, 40)];
                                                                 logoImageView.image = [UIImage imageNamed:@"menuLogo.jpg"];
                                                                 UIView *controllerView = myAlertController.view;
