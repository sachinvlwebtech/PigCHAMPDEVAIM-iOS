...
//        [tracker send:[[GAIDictionaryBuilder createScreenView] build]];
        self.navigationController.navigationBar.translucent = NO;
        {
            ...
            strTitle = @"Animal Status";
            
            NSArray* resultArray1 = [[CoreDataHandler sharedHandler] getTranslatedText:[[NSMutableArray alloc] initWithObjects:@"OK",@"Please Wait...",@"You must be online for the app to function.",@"Identity does not exists.",@"Your session has been expired. Please login again.",@"Server Error.",@"Signing off.",@"Animal Status",nil]];
                    ...
                    if (i==0){
                        ...
                        if ([dictMenu objectForKey:[@"Identity does not exists." uppercaseString]] && ![[dictMenu objectForKey:[@"Identity does not exists." uppercaseString]] isKindOfClass:[NSNull class]]) {
                            if ([[dictMenu objectForKey:[@"Identity does not exists." uppercaseString]] length]>0) {
                                strIdentityMsg = [dictMenu objectForKey:[@"Identity does not exists." uppercaseString]]?[dictMenu objectForKey:[@"Identity does not exists." uppercaseString]]:@"";
                            }
                        }
                    }else  if (i==4){
                        ...
                        if ([dictMenu objectForKey:[@"Animal Status" uppercaseString]] && ![[dictMenu objectForKey:[@"Animal Status" uppercaseString]] isKindOfClass:[NSNull class]]) {
                            if ([[dictMenu objectForKey:[@"Animal Status" uppercaseString]] length]>0) {
                                strTitle = [dictMenu objectForKey:[@"Animal Status" uppercaseString]]?[dictMenu objectForKey:[@"Animal Status" uppercaseString]]:@"";
                            }
                        }
                    }
                }
    ...
    @try {
        ...
        if (![[[arrActiveAnimalList objectAtIndex:indexPath.row] valueForKey:@"prop"] isKindOfClass:[NSNull class]]){
           // lblName.text = [[arrActiveAnimalList objectAtIndex:indexPath.row] valueForKey:@"prop"]?[[arrActiveAnimalList objectAtIndex:indexPath.row] valueForKey:@"prop"]:@"";
            lblName.text = [[arrActiveAnimalList objectAtIndex:indexPath.row] valueForKey:@"translatedValue"]?[[arrActiveAnimalList objectAtIndex:indexPath.row] valueForKey:@"translatedValue"]:@"";
        }
...
//                                                                                    preferredStyle:UIAlertControllerStyleAlert];
                ...
                UIView *controllerView = myAlertController.view;
                [controllerView addSubview:logoImageView];
                [controllerView bringSubviewToFront:logoImageView];UIAlertAction* ok = [UIAlertAction
                                     actionWithTitle:strOK
                                     style:UIAlertActionStyleDefault
                                     handler:^(UIAlertAction * action) {
                                         //[self.navigationController popToRootViewControllerAnimated:YES];
